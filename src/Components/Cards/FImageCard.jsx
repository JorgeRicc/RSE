...
/* 
    ...
    url: string url,
*/
const FImageCard = ({ title, date, preview, image, imageAlt, color, url }) => {
        ...
        <div className={`p-4 md:p-12 lg:p-4 rounded-2xl h-full transition-all cursor-pointer shadow-xl`} onClick={onClickHandler}>
                ...
                <div className="lg:grid lg:grid-cols-12 mx-auto">
                    <div className="lg:col-span-5 lg:m-auto lg:pr-4 mx-auto">
                        <img src={image} alt={imageAlt} className={`mx-auto rounded-xl border border-${color}-700 group-hover:border-gray-500`} />
                    </div>
                    <div className="lg:col-span-7 lg:m-auto">
                        <h1 className="text-3xl md:text-5xl text-center my-2">{title}</h1>
                        <h3 className={`text-xl md:text-2xl text-center my-1 bg-white text-black border-${color}-700 group-hover:border-gray-500 rounded-xl w-fit p-1 m-auto`}>{date}</h3>
                    </div>
                </div>
